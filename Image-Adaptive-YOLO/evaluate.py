...
if __name__ == '__main__':  
    image_path = "/home/yj/Desktop/yolo/Image-Adaptive-YOLO/data/foggyimages/frame_055.jpg"
    image = cv2.imread("/home/yj/Desktop/yolo/Image-Adaptive-YOLO/data/foggyimages/frame_055.jpg")
    image_name = image_path.split('/')[-1]
    y = YoloTest()
    #YoloTest().predict(image,image_name)
    bboxes_pr, image_isped, time_one_img = y.predict(image,image_name)
    #num_img += 1
    #time_total_cnn_process_img += time_one_img
    #time_total += time.time() - t1
    ...
    if y.write_image:
        if y.isp_flag:
            image = utils.draw_bbox(image_isped, bboxes_pr, y.classes, show_label=y.show_label)
        else:
            image = utils.draw_bbox(image_isped, bboxes_pr, y.classes, show_label=y.show_label)
        cv2.imwrite(y.write_image_path+image_name, image)
    ...
    '''with open(predict_result_path, 'w') as f:
        for bbox in bboxes_pr:
            ...
            score = bbox[4]
            class_ind = int(bbox[5])
            class_name = y.classes[class_ind]
